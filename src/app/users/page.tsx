...
import { Plus, Trash2, Edit, Search, RefreshCw, X, Shield, UserCheck, UserX, Key, Calendar, Lock } from 'lucide-react'
...
const defaultPermissoes = () =>
  modulos.reduce((acc, m) => ({
    ...acc,
    [m.key]: { ver: true, criar: false, editar: false, deletar: false }
...
export default function UsersPage() {
  ...
  const [permissoes, setPermissoes] = useState<Record<string, Record<string, boolean>>>(defaultPermissoes())
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')
  const [showSenha, setShowSenha] = useState(false)
  const [abaModal, setAbaModal] = useState<'dados' | 'permissoes'>('dados')
  ...
  const openEdit = (item: Record<string, any>) => {
    ...
    setPermissoes(item.permissoes ? { ...defaultPermissoes(), ...item.permissoes } : defaultPermissoes())
    setShowSenha(false)
    setAbaModal('dados')
    setError('')
    setShowModal(true)
  ...
  const handleToggleStatus = async (item: Record<string, any>) => {
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#243324] bg-[#0d160d]">
                  ...
                  <th className="text-left py-3 px-4 text-green-600 font-medium">Permissões</th>
                    ...
                    <tr key={item.id} className="border-b border-[#1a251a] hover:bg-[#1a251a]/50 transition-colors">
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-xs text-green-600">{modulosAtivos}/{modulos.length} módulos</span>
                        </div>
                      </td>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-[#111811] border border-[#243324] rounded-2xl w-full max-w-2xl shadow-2xl">
            ...
            <div className="flex border-b border-[#243324]">
              ...
              <button onClick={() => setAbaModal('permissoes')}
                className={`flex-1 py-3 text-sm font-medium transition-colors border-b-2 -mb-px ${abaModal === 'permissoes' ? 'border-green-500 text-green-400' : 'border-transparent text-green-700 hover:text-green-400'}`}>
            ...
            <div className="p-6 max-h-[60vh] overflow-y-auto">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="col-span-2">
                      ...
                      <input value={form.nome || ''} onChange={e => setForm({...form, nome: e.target.value})}
                        className="w-full bg-[#1a251a] border border-[#243324] rounded-xl px-4 py-2.5 text-green-200 text-sm outline-none focus:border-green-600"
              ...
              {abaModal === 'permissoes' && (
                <div className="space-y-3">
                  <p className="text-green-600 text-xs mb-4">Configure quais módulos e ações este usuário pode acessar.</p>
                  <div className="grid grid-cols-5 gap-2 mb-2">
                    <div className="text-green-600 text-xs font-medium">Módulo</div>
                    {acoes.map(a => (
                      <div key={a} className="text-green-600 text-xs font-medium text-center capitalize">{a}</div>
                    ))}
                  </div>
                  {modulos.map(m => (
                    <div key={m.key} className="bg-[#1a251a] rounded-xl p-3 border border-[#243324]">
                      <div className="grid grid-cols-5 gap-2 items-center">
                        <button onClick={() => toggleModulo(m.key)}
                          className="text-green-300 text-sm font-medium text-left hover:text-green-100 transition-colors">
                          {m.label}
                        </button>
                        {acoes.map(a => (
            ...
            <div className="flex gap-3 p-6 border-t border-[#243324]">
              <button onClick={() => setShowModal(false)}
                className="flex-1 px-4 py-2.5 border border-[#243324] text-green-600 rounded-xl text-sm">
                Cancelar
              </button>
              <button onClick={handleSave} disabled={saving}
                className="flex-1 px-4 py-2.5 bg-green-600 hover:bg-green-500 text-white rounded-xl text-sm font-medium disabled:opacity-50 flex items-center justify-center gap-2">
