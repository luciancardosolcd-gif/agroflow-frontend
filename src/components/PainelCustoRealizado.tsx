...
function DonutChart({ dados, total, tipo }: { dados: CategoriaResumo[]; total: number; tipo: 'receita' | 'despesa' }) {
  ...
  const fatias = dados
    ...
    .map((d, i) => {
      ...
      const dashoffset = circunferencia - offset;
      offset += comprimento;
      return {
        ...d,
        comprimento,
  ...
  return (
    <div className="relative flex items-center justify-center">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
        ...
        <span className="text-sm font-bold text-gray-100 leading-tight text-center px-4">
          {formatBRL(total)}
        </span>
      </div>
    </div>
...
export default function PainelCustoRealizado({ token, safraId, fazendaId }: PainelCustoRealizadoProps) {
  ...
  useEffect(() => {
    ...
    if (periodo.inicio) params.set('startDate', periodo.inicio);
    if (periodo.fim) params.set('endDate', periodo.fim);
            ...
            {lista.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-32 gap-2">
                ...
                <p className="text-gray-600 text-sm">
                  Nenhum lançamento com categoria
                </p>
                <p className="text-gray-700 text-xs text-center">
                  Vincule categorias aos lançamentos<br/>para ver o custo realizado
                </p>
              </div>
            ) : (
              lista.map((cat, i) => (
                <BarraCategoria
                  ...
                  cat={cat}
                  total={total}
                  cor={cat.color || cores[i % cores.length]}
                />
              ))
            )}
          </div>
